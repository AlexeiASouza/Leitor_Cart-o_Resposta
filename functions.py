...
#Função para obtenção do objeto recortado da imagem
...
#Função para calcular o centro da bolha (template)
...
#Função para calcular as coordenadass da bolha  (template)
...
def points_inside_circle(raio,centro,pp):
    pontos = []
...
def points_in_perimeter(raio, xc, yc):
    pontos_perimetro = []

    # Inicialização dos pontos de controle
    x = 0
    y = raio
    d = 3 - 2 * raio

    # Desenho da circunferência utilizando o algoritmo de Bresenham
    while x <= y:
        # Adiciona os pontos refletindo as oito octantes
        pontos_perimetro.append((xc + x, yc + y))
        pontos_perimetro.append((xc + x, yc - y))
        pontos_perimetro.append((xc - x, yc + y))
        pontos_perimetro.append((xc - x, yc - y))
        pontos_perimetro.append((xc + y, yc + x))
        pontos_perimetro.append((xc + y, yc - x))
        pontos_perimetro.append((xc - y, yc + x))
        pontos_perimetro.append((xc - y, yc - x))
...
def get_circle_center(size,r,img):
    circles = []
    for i in range(r,size - r):
        for j in range(r,size -r):
            xc = i
            yc = j
            points = points_in_perimeter(r,xc,yc)
            check = check_points(points,img)
            if check > 0:
                circles.append([check,(xc,yc)])
    return sorted(circles)[::-1][0][1]

def process_rois(region, vertices,l_mask,l_roi):
    rois = get_rois(region,vertices,l_mask,l_roi)
    result_rois = []
    r = bubble_diameter//2
    for roi in rois:
        xc,yc = get_circle_center(x_bubble_space+2,r,roi)
        inside_points = points_inside_circle(r,(xc,yc),[])
        per_points = points_in_perimeter(r,xc,yc)
        ones = np.ones_like(roi)*255
        for in_point in inside_points:
            ones[in_point[0]][in_point[1]] = roi[in_point[0]][in_point[1]]

        for per_point in per_points:
            ones[per_point[0]][per_point[1]] = 0

        result_rois.append(ones)
    return result_rois
...
def get_segmentation_dict(roi,r,xc,yc,nvalue):
    ...
    for i in range(r):
        points = points_inside_circle(i+1,(xc,yc),previous_points)
...
def get_results(rois):
    ...
    for roi in rois:
        try:
            xc,yc = get_circle_center(x_bubble_space+2,bubble_diameter//2,roi)
...
def get_info(img,detection_model):
    ...
    _,result =  cv2.threshold(result,128,255,cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    area = grade_area
    rectangles = get_rectangles(result,area)
    rectangles = [sorted(rectangles,key = cv2.contourArea)[0]]
    result_rectangle = get_result_rectangle(rectangles)
    
    #Obtém a grade e a inclinação da página
    result_grade,_ = get_grade(result,result_rectangle)
    ...
    for vertices in grade_vertices:
        result_rois = process_rois(result_grade,vertices,bubble_diameter,x_bubble_space)
